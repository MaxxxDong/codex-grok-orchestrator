...
`grok-worker` **0.5.3** adds event-first `watch`, immediate terminal/attention notifications, and a one-pass `preflight` command that reports every blocked path without exposing matched values. Startup failures now notify an already waiting dispatcher, while the credential scanner avoids treating long runtime identifiers as literal secrets. Native one-shot work continues to use the user's normal `~/.grok`, preserving configured plugins, MCP servers, OAuth state, High reasoning, and provider prompt caching. The Windows-native integration retains hidden process creation, Win32 process-tree cleanup, managed acpx compatibility, UTF-8 handling, and configurable dispatcher capacity without using WSL. See the [release notes](docs/releases/release-notes.md) and [Windows native upgrade guide](docs/windows-upgrade.md).
...
| Windows | 支持（Windows native 集成分支） | Grok 0.2.103 原生 Headless；不使用 WSL 或全局 acpx 回退 |
...
- Windows 命名会话与显式 ACP 兼容路径使用 grok-worker 托管的 `acpx`；不回退全局 acpx 或 WSL
- 已独立登录的 Grok CLI
- 源码启动器或共享依赖准备时需要 `uv`
...
`run` 在所有平台默认使用原生 Headless；Windows 已用 Grok Build 0.2.103 验证终端与文件工具链。ACP 仍可显式选择，并继续承载命名会话。实现任务使用 `--mode implementation`。普通 staged、unstaged、untracked 文件会经过敏感内容与软链接检查后自动快照；ignored 文件不会复制，疑似密钥与越界软链接仍拒绝启动。旧 `--include-dirty` / `--include-dirty-path` 只作兼容，不再是启动门槛，也不再过滤其他安全脏文件。
...
- [Windows native 0.3-0.5.2 → 0.5.3 升级](docs/windows-upgrade.md)
- [Skill 接入](SKILL.md)
...
| Windows | Supported by the Windows-native integration branch | Grok 0.2.103 native headless; no WSL/global-acpx fallback |
...
- On Windows, named sessions and the explicit ACP compatibility path use the grok-worker-managed `acpx`; no global-acpx or WSL fallback
...
`run` defaults to native headless on every platform; Windows terminal and file tools are verified with Grok Build 0.2.103. ACP remains explicitly selectable and continues to power named sessions. Use `--mode implementation` for edits. Ordinary staged, unstaged, and untracked files are automatically snapshotted after sensitive-content and symlink checks. Ignored files are never copied; suspected secrets and escaping symlinks still block startup. Legacy dirty flags remain accepted but no longer filter other safe dirt.
...
- [Windows native 0.3-0.5.2 → 0.5.3 upgrade](docs/windows-upgrade.md)
