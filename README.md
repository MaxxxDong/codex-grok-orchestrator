...
`grok-worker` **0.5.2** uses the user's normal `~/.grok` for native one-shot work, preserving configured plugins, MCP servers, OAuth state, High reasoning, and provider prompt caching. Startup inspection is advisory, mutable package caches stay inside the disposable clone, prepared environments remain shared, and clone-owned Grok sessions are removed after completion. On the Windows-native integration branch, the proven managed-ACP tool chain remains the one-shot default because the current Native workspace sandbox can reject terminal processes; `--backend native` remains available explicitly. Windows also retains managed acpx, PowerShell 7/UTF-8, hidden child processes, process-tree cleanup, and configurable dispatcher capacity. See the [release notes](docs/releases/release-notes.md) and [Windows native upgrade guide](docs/windows-upgrade.md).
...
| Windows | 支持（Windows native 集成分支） | 默认托管 ACP；不使用 WSL 或全局 acpx 回退 |
...
- Windows 默认 ACP 和命名会话使用 grok-worker 托管的 `acpx`；不回退全局 acpx 或 WSL
- 已独立登录的 Grok CLI
- 源码启动器或共享依赖准备时需要 `uv`
...
`run` 在 Windows 默认使用已验证的托管 ACP 链路，其他平台默认使用原生 Headless；两种后端都可显式选择。实现任务使用 `--mode implementation`。普通 staged、unstaged、untracked 文件会经过敏感内容与软链接检查后自动快照；ignored 文件不会复制，疑似密钥与越界软链接仍拒绝启动。旧 `--include-dirty` / `--include-dirty-path` 只作兼容，不再是启动门槛，也不再过滤其他安全脏文件。
...
- [Windows native 0.3/0.4/0.5.1 → 0.5.2 升级](docs/windows-upgrade.md)
- [Skill 接入](SKILL.md)
...
| Windows | Supported by the Windows-native integration branch | Managed ACP by default; no WSL/global-acpx fallback |
...
- On Windows, the default ACP path and named sessions use the grok-worker-managed `acpx`; no global-acpx or WSL fallback
...
- [Windows native 0.3/0.4/0.5.1 → 0.5.2 upgrade](docs/windows-upgrade.md)
