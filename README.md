...
| Windows | 支持（Windows native 集成分支） | 原生运行；托管 acpx 与无窗口进程策略见[运维文档](docs/operations.md#native-windows-runtime) |
...
- 仅 `--backend acp` 和命名会话需要兼容 ACP 的 `acpx` 客户端；Windows 使用受管 runtime，不回退到全局 acpx 或 WSL
- 已独立登录的 Grok CLI
- 源码启动器或共享依赖准备时需要 `uv`
...
- [Windows native 0.3/0.4 → 0.5.1 升级](docs/windows-upgrade.md)
- [Skill 接入](SKILL.md)
...
| Windows | Supported by the Windows-native integration branch | Native runtime with managed acpx and no-window process policy; see [Operations](docs/operations.md#native-windows-runtime) |
...
- An `acpx`-compatible client only for `--backend acp` and named sessions; Windows uses the managed runtime without global-acpx or WSL fallback
...
- [Windows native 0.3/0.4 → 0.5.1 upgrade](docs/windows-upgrade.md)
