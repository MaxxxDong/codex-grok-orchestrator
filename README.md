...
The cross-platform **0.8.0** release removes the model-turn cap, continues
...
final gates, durable per-run receipts, multi-root health, bounded dirty
...
[macOS](docs/macos-upgrade.md) and
[Windows native](docs/windows-upgrade.md) upgrade guides. Windows additionally
retains hidden process creation, Win32 process-tree cleanup, managed acpx
compatibility, and PowerShell 7/UTF-8 handling without using WSL.

### Version history / 版本演进

| Version | Main update / 核心更新 |
|---|---|
| [`0.8.0 macOS`](https://github.com/MaxxxDong/codex-grok-orchestrator/tree/codex/macos-native-v0.8.0) · [`Windows`](https://github.com/MaxxxDong/codex-grok-orchestrator/tree/codex/windows-native-v0.8.0) | Same-session budget recovery, one-shot settled watch, runner-owned gates, durable receipts, multi-root health, and cross-platform hardening. / 同会话预算恢复、一次等待至清理完成、runner 门禁、持久回执、多根健康检查与跨平台加固。 |
...
| Windows | 支持（Windows native 集成分支） | Grok 0.2.111 原生 Headless；不使用 WSL 或全局 acpx 回退 |
...
仍可省略 `--detach`。macOS 与 Windows 已用 Grok Build 0.2.111 验证终端与文件工具链；
需要旧通信层或命名会话时显式传 `--backend acp`。实现任务使用
...
- [macOS 0.3-0.7.2 → 0.8.0 升级](docs/macos-upgrade.md)
...
| Windows | Supported by the Windows-native integration branch | Grok 0.2.111 native headless; no WSL/global-acpx fallback |
...
Omit `--detach` only for direct interactive use or launcher diagnosis. macOS and
Windows terminal and file tools are verified with Grok Build 0.2.111. Use `--backend acp`
only for compatibility or named sessions, and `--mode implementation` for edits.
...
- [macOS 0.3-0.7.2 → 0.8.0 upgrade](docs/macos-upgrade.md)
