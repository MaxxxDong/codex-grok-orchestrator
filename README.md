...
Worker that may recover. Launcher logs stay in the bounded shared cache. The
Windows-native integration retains hidden process creation, Win32 process-tree
cleanup, managed acpx compatibility, PowerShell 7/UTF-8 handling, and the single
native provider configuration without using WSL. See the [release notes](docs/releases/release-notes.md)
and [Windows native upgrade guide](docs/windows-upgrade.md).
...
| Windows | 支持（Windows native 集成分支） | Grok 0.2.106 原生 Headless；不使用 WSL 或全局 acpx 回退 |
...
仍可省略 `--detach`。Windows 已用 Grok Build 0.2.106 验证终端与文件工具链；
...
- [Windows native 0.3-0.5.3 → 0.6.0 升级](docs/windows-upgrade.md)
- [Skill 接入](SKILL.md)
...
| Windows | Supported by the Windows-native integration branch | Grok 0.2.106 native headless; no WSL/global-acpx fallback |
...
Omit `--detach` only for direct interactive use or launcher diagnosis. Windows
terminal and file tools are verified with Grok Build 0.2.106. Use `--backend acp`
...
- [Windows native 0.3-0.5.3 → 0.6.0 upgrade](docs/windows-upgrade.md)
