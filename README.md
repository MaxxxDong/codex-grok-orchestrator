...
| Windows | 支持 | 原生运行；使用受管 acpx runtime，详见[运维文档](docs/operations.md#native-windows-runtime) |

### 环境要求

- Python 3.12+
- Git
- `PATH` 中兼容 ACP 的 `acpx` 客户端；Windows 首次运行前需按[运维文档](docs/operations.md#native-windows-runtime)安装受管 runtime
- 已独立登录的 Grok CLI
- 源码启动器或共享依赖准备时需要 `uv`
...
同一会话的任务 ID、源仓库、基线提交、角色、模式与权限签名必须一致。
...
| Windows | Supported | Native runtime; see [Operations](docs/operations.md#native-windows-runtime) |

### Requirements

- Python 3.12+
- Git
- An `acpx`-compatible ACP client on `PATH`; Windows requires the managed runtime setup in [Operations](docs/operations.md#native-windows-runtime)
...
Task ID, source repository, baseline commit, role, mode, and permission signature stay immutable for a session.
